...
- Next.js: 管理画面を静的出力
- Cloudflare Pages Functions / Workers: NFC広告ページ、棚前ディスプレイ、管理API、認証
- Cloudflare Durable Objects: 棚前ディスプレイへのリアルタイムWebSocket通知
- Cloudflare D1: NFCタグ、広告キャンペーン、棚前画面、匿名ユーザー、ポイント台帳、日次集計、秒単位履歴
...
- 棚前ディスプレイ: [http://127.0.0.1:8788/screen/onion-shelf](http://127.0.0.1:8788/screen/onion-shelf)
...
- 棚前ディスプレイ: [https://kazashitego.kazashitego-go.workers.dev/screen/onion-shelf](https://kazashitego.kazashitego-go.workers.dev/screen/onion-shelf)
...
- 棚前ディスプレイは `/screen/onion-shelf` を開きっぱなしにします。
- スマホでNFCをかざすと、スマホ側は今まで通りポイント画面を返します。
- 同時に、Cloudflare Durable ObjectのWebSocketで棚前ディスプレイへ広告切り替えを通知します。
- `kg-0001` / `kg-0002` / `kg-0003` は、初期状態では同じ `onion-shelf` 画面へ通知します。
- 連続切り替えで画面が荒れないよう、棚前画面ごとに4秒のクールタイムを入れています。
...
wrangler d1 execute kazashitego-db --remote --file=./migrations/0006_add_screen_signage.sql
